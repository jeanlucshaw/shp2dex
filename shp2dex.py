...
def load_cis_shp(name, ascending=True):
    ...
    ascending: bool
        Passed to pandas.sort_values, sort shapes by ascending
        or descending polygon area.
...
def plot_cis_shp(sname, target=None, suffix=None, decimate=10, **ax_kw):
    """
    ...
    target: int or iterable of int
        Records to plot.
    suffix: str
        If provided, the figure is saved with this string before
        the png extension.
    decimate: int
        Plot 1/`decimate` points for polygon vectors longer than
        10000 points.
    ax_kw: dict
    ...
    df_size = df_managed.LEGEND.size

    # Init plot
    gs_kw = dict(left=0.2, bottom=0.15)
    _, ax = plt.subplots(figsize=(4, 3.5), gridspec_kw=gs_kw)
    ...
    for (i, shape) in enumerate(df_records.shapes.values):
        ...
        if isinstance(decimate, int) and lon.size > 10_000:
            lon, lat = lon[::decimate], lat[::decimate]
    ...
    # Save if suffix provided
    if suffix:
        plt.savefig('%s.%s.png' % (sname[:-4], suffix))
    plt.show()
...
def _manage_shapefile_types(dataframe):
    ...
    # Ensure the legend field is marked as water and not missing
    # if 'X' in dataframe.LEGEND.unique():
    #     condition = 'LEGEND == CT == CA == SA == FA == "X"'
    #     target = dataframe.query(condition).index
    #     dataframe.loc[target, 'LEGEND'] = 'W'
